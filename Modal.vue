		...
		>
			...
			>
				<q-header class="bg-primary">
					<q-toolbar>
						...
						<q-btn flat v-if="hasBtnConfirm" :disabled="loader" @click="onConfirm()" dense>{{ $t("modals.confirm") }}</q-btn>
					</q-toolbar>
				</q-header>
				<q-page-container>
					<q-page padding>
			...
			<q-card v-else style="width: 700px; max-width: 80vw">
				...
				<q-card-actions align="right">
					<q-btn flat @click="onCancel()" :disabled="loader" :label="$t('modals.cancel')" color="secondary"/>
					<q-btn flat v-if="hasBtnConfirm" @click="onConfirm()" :disabled="loader" :label="$t('modals.confirm')" color="secondary"/>
				</q-card-actions>
			</q-card>
		</q-dialog>
	</div>
